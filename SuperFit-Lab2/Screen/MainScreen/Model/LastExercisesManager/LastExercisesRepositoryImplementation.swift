...
final class LastExercisesRepositoryImplementation: LastExercisesRepository {
    ...
    func saveLastExercise(trainingType: TrainingTypeModel) {
        ...
        let currentLastExerciseName = getTrainingTypeModelByTrainingNameUseCase.getTrainingTypeModel(
            by: fetchLastExercise()
        )?.rawValue
    ...
    func isThereSavedExercises() -> Bool {
        if !fetchLastExercise().isEmpty || !fetchPreLastExercise().isEmpty {
            return true
        }
        return false
    }
    ...
    func getLastExercises() -> [TrainingTypeModel] {
        ...
        if let preLastExercise = getTrainingTypeModelByTrainingNameUseCase.getTrainingTypeModel(
            by: preLastExerciseName
        ) {
