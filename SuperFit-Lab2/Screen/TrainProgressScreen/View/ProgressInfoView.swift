...
final class ProgressInfoView: UIView {
    ...
    func configure(with trainingProgress: TrainingProgressModel?) {
        if trainingProgress != nil {
            let lastTrainLabelAttributedString = NSMutableAttributedString(
                string: "\(R.string.trainProgressScreen.last_train()) ",
                attributes: [
                    NSAttributedString.Key.foregroundColor: R.color.white()!,
                    NSAttributedString.Key.font: R.font.montserratRegular(size: 12)!
                ]
            )
            lastTrainLabelAttributedString.append(
                NSAttributedString(
                    string: trainingProgress!.lastTrain,
                    attributes: [
                        NSAttributedString.Key.foregroundColor: R.color.white()!,
                        NSAttributedString.Key.font: R.font.montserratBold(size: 12)!
                    ]
                )
            )
            ...
            let progressLabelAttributedString = NSMutableAttributedString(
                string: "\(R.string.trainProgressScreen.progress()) ",
                attributes: [
                    NSAttributedString.Key.foregroundColor: R.color.white()!,
                    NSAttributedString.Key.font: R.font.montserratRegular(size: 12)!
                ]
            )
            progressLabelAttributedString.append(
                NSAttributedString(
                    string: (!trainingProgress!.progress.isEmpty) ? trainingProgress!.progress : "N/A%",
                    attributes: [
                        NSAttributedString.Key.foregroundColor: R.color.white()!,
                        NSAttributedString.Key.font: R.font.montserratBold(size: 12)!
                    ]
    ...
    private lazy var progressLabel: UILabel = {
        let myLabel = UILabel(
            frame: CGRect(
                x: lastTrainLabel.frame.minX,
                y: lastTrainLabel.frame.maxY + 5,
                width: 200,
                height: 100
            )
        )
    ...
    private lazy var progressArrowImageView: UIImageView = {
        let myImageView = UIImageView(
            frame: CGRect(
                x: Int(progressLabel.frame.maxX + 8),
                y: Int(progressLabel.frame.minY),
                width: 8,
                height: 14
            )
        )
